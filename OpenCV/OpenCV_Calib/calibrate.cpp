...
int main()
{
   ...
   for (int i=1; i<=num; i++) {
		...
		// str << "new/chessboard" << std::setw(2) << std::setfill('0') << i << ".jpg";
	...
	// add the corners from the chessboard,棋盘格内部角点数量
	cv::Size boardSize(7,5);
	// cv::Size boardSize(9,6);
	cameraCalibrator.addChessboardPoints(
		filelist,	// filenames of chessboard image
		boardSize, "Detected points");	// size of chessboard，标定面板的大小
	...
	cv::FileStorage fs("/home/deploy/4Dlabel/Calibra/OpenCV_Chapter11/build/NewCalib.xml", cv::FileStorage::WRITE);
