...
#include "CameraCalibratorClass.h"
...
int CameraCalibrator::addChessboardPoints(
    ...
    for (int i=0; i<filelist.size(); i++) {
        ...
        // Get the chessboard corners,获取棋盘格角点
		...
		if (windowName.length()>0 && imageCorners.size() == boardSize.area()) {
			...
			//Draw the corners，画出角点
			// std::cout<<"画出角点"<<std::endl;
...
double CameraCalibrator::calibrate(const cv::Size imageSize)
{
   ...
   bool mustInitUndistort_test= true;
...
cv::Mat CameraCalibrator::remap(const cv::Mat &image,const cv::Size &outputSize) {
cv::Mat undistorted;

	// if (outputSize.height == -1)// todo
	// 	outputSize = image.size();

	if (mustInitUndistort) { // called once per calibration
	   cameraMatrix.at<double>(0,0)=cameraMatrix.at<double>(0,0)*1;
	   cameraMatrix.at<double>(1,1) =cameraMatrix.at<double>(1,1) *1;
	   cameraMatrix.at<double>(0,2)=cameraMatrix.at<double>(0,2)+0;
	   cameraMatrix.at<double>(1,2)=cameraMatrix.at<double>(1,2)+200;
