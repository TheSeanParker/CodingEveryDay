...
int main(int argc, char* argv[]) {
  ...
  // 起点
  Pose3 pose_0(Rot3::Rodrigues(0, 0, 0), Point3(0,0,0));
  ...
  for (size_t i = 0; i < 3; ++i) 
  {
        ...
        if (i > 0) 
        {
                ...
                if (i % 2 == 0) {//取余数
                  ...
                  // newgraph.add(f);
                  newgraph.add(BetweenFactor<imuBias::ConstantBias>(b1, b2, imuBias::ConstantBias(), cov));
                  // 将当前的IMU偏差变量的初始估计加入初始值集合
                  initialEstimate.print("before insert----------------");
                  initialEstimate.insert(b2, imuBias::ConstantBias());
          ...
          // newgraph.resize(0);
          // initialEstimate.clear();// 上面2行会将之前的数据清空
  }
  // 使用初始值保存因子图
  ofstream file;
  file.open("file.dot");
  ...
  initialEstimate.print("--------------end  It initialEstimate=");
