...
{
    /* code */
    //将所有行都注释掉，然后从上到下逐一解注释，得到的代码，再去逐一测试
    Base1 b1;
    Sub1  s1;
    //上下两行代码均调用了拷贝构造函数
    // Base1 b2(s1);
    process1(s1);//形参输入子类，那么将会调用子类的函数，而不是父类的函数
                 //call Sub func
                 //形参和实参的赋值，隐含调用拷贝赋值运算
    Sub1* ps1=new Sub1();//new出一个对象，隐式调用Base1的构造函数

    process2(ps1);//process2(&s1);
                  //call Sub func
    Base1* ps2=new Sub1();  //先生成一个Sub1类型的对象，然后再赋值给左侧的指针变量
    process2(ps2);//call Sub1 func
    delete ps1;
    delete ps2;
    process3(s1) ;//call Sub1 func
    return 0;
} 
